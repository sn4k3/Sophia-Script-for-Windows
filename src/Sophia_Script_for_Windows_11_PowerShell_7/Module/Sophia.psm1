...
"@
	...
	# Save in UTF8 without BOM
	Add-Content -Path "$env:TEMP\LGPO.txt" -Value $Policy -Encoding utf8NoBOM -Force
}
...
function TaskbarWidgets
{
	...
	Set-Policy -Scope Computer -Path SOFTWARE\Policies\Microsoft\Dsh -Name AllowNewsAndInterests -Type DELETE
			...
			else
			{
				try
				{
					...
					if ($AllUsers)
					{
						...
						Start-Process -FilePath "$DownloadsFolder\OneDriveSetup.exe" -ArgumentList "/allusers"
					}
					else
					{
...
function DefaultTerminalApp
{
	...
	switch ($PSCmdlet.ParameterSetName)
	{
		"WindowsTerminal"
		{
			...
			if ((Get-AppxPackage -Name Microsoft.WindowsTerminal) -and ([System.Version]$TerminalVersion -ge [System.Version]"1.11"))
			{
	...
	switch ($PSCmdlet.ParameterSetName)
	{
		...
		"ComssOneDNS"
		{
			...
			if (-not (Get-CimInstance -ClassName CIM_ComputerSystem).HypervisorPresent)
			{
				...
				{
					Get-NetAdapter -Physical | Get-NetIPInterface -AddressFamily IPv4 | Set-DnsClientServerAddress -ServerAddresses $FirstIPAddress
				}
			}
			else
			{
				...
				{
					Get-NetRoute | Where-Object -FilterScript {$_.DestinationPrefix -eq "0.0.0.0/0"} | Get-NetAdapter | Set-DnsClientServerAddress -ServerAddresses $FirstIPAddress
				}
			}
